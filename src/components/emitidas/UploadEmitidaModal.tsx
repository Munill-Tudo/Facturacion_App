...
export function UploadEmitidaModal({
  onClose,
  onSuccess,
  initialFile = null,
}: {
  ...
  const processAll = useCallback(async (fileItems: FileItem[]) => {
    ...
    for (const item of pending) {
      updateItem(item.id, { status: 'uploading', message: 'Procesando con IA (Emitida)…' });
      try {
        const fd = new FormData();
        fd.append('file', item.file);
        const res = await fetch('/api/emitidas/upload', { method: 'POST', body: fd });
        ...
        updateItem(item.id, {
          ...
          cliente: data.factura?.cliente || 'Cliente',
        });
      } catch (err: any) {
  ...
  const statusIcon = (s: FileStatus) => {
    if (s === 'uploading') return <Loader2 className="w-4 h-4 text-emerald-500 animate-spin shrink-0" />;
  ...
  const statusColor = (s: FileStatus) => {
    if (s === 'uploading') return 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/30';
        ...
        {!allDone && (
          <div className="p-5 shrink-0">
            <div
              ...
              className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer select-none ${
                isDragging
                  ...
                  : 'border-gray-300 dark:border-gray-700 hover:border-emerald-400 hover:bg-gray-50 dark:hover:bg-white/5'
            ...
            >
              ...
              />
              <UploadCloud className={`w-8 h-8 mx-auto mb-2 transition-colors ${isDragging ? 'text-emerald-500' : 'text-gray-400'}`} />
              <p className="font-semibold text-gray-900 dark:text-white text-sm">
                {isDragging ? '¡Suelta para añadir!' : 'Arrastra PDFs de TUS FACTURAS aquí'}
              </p>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.message && (
                    <p className={`text-xs mt-0.5 ${
                      ...
                      {item.status === 'done' && item.cliente ? `${item.cliente} · ` : ''}{item.message}
                    </p>
                  )}
                </div>
          ...
          <div className="px-5 py-4 shrink-0">
            <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 flex items-center gap-3">
              ...
              <div>
                ...
                {errorCount > 0 && <p className="text-xs text-red-500 mt-0.5">{errorCount} error(es)</p>}
              </div>
            </div>
          </div>
        )}
        ...
        <div className="p-5 border-t border-gray-100 dark:border-gray-800 flex justify-between gap-3 bg-gray-50/50 dark:bg-white/5 shrink-0">
          {allDone ? (
            <>
              ...
              <button onClick={onClose} className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold ml-auto">
                Cerrar
              </button>
            </>
          ) : (
            <>
              <button onClick={onClose} disabled={isRunning} className="px-5 py-2.5 rounded-xl text-gray-600 hover:bg-gray-200 transition-colors font-medium">
                Cancelar
              </button>
              <button
                onClick={() => processAll(items)}
                disabled={isRunning || items.filter(it => it.status === 'pending').length === 0}
                className="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold flex items-center gap-2 disabled:opacity-50">
                {isRunning ? <><Loader2 className="w-4 h-4 animate-spin" />Procesando…</> : <><UploadCloud className="w-4 h-4" />Subir {items.filter(it => it.status === 'pending').length}</>}
              </button>
            </>
          )}
        </div>
