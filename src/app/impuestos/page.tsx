    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-lg bg-white dark:bg-[#0f0f0f] rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 animate-in slide-in-from-bottom-4 duration-300 overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          <div>
            ...
            <input
              ...
              placeholder="ej: IVA 1T, IRPF Anual, IAE..."
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="ej: 1T 2025, Enero 2026..."
                className="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-black text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 text-sm"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={e => set('fecha_devengo', e.target.value)}
                className="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-black text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/30 text-sm"
              />
            </div>
            <div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-end gap-3">
          ...
          <button
            onClick={() => {
              if (!form.concepto || !form.periodo || !form.importe || !form.fecha_devengo) return;
...
function VincularModal({
  ...
  const filtered = useMemo(() => {
    ...
    return movimientos
      ...
      .sort((a, b) => {
        // Primero los que coinciden en importe exacto
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {impuesto.concepto} — {fmt(impuesto.importe)}
            </p>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 text-gray-400 transition-colors">
            <X className="w-5 h-5" />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-sm bg-white dark:bg-[#0f0f0f] rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 p-6 animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 font-medium text-sm hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
            Cancelar
          </button>
          <button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-sm transition-colors shadow-lg shadow-red-500/20">
            Eliminar
          </button>
        </div>
      </div>
...
export default function ImpuestosPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (errImp) {
        if (errImp.code === '42P01') {
          setDbError('La tabla "impuestos" no existe. Ejecuta el SQL de configuración en Supabase.');
        } else {
          setDbError(errImp.message);
        }
        setLoading(false);
        return;
      }
  ...
  const handleSave = async (data: ImpuestoInput) => {
    ...
    try {
      if (editing) {
        await actualizarImpuesto(editing.id, data);
        setEditing(null);
      } else {
  ...
  const handleVincular = async (movimientoId: number) => {
    ...
    setIsSaving(true);
    try {
      await vincularMovimiento(vinculando.id, movimientoId);
      setVinculando(null);
      await fetchData();
    } finally {
      setIsSaving(false);
  ...
  const handleAutoConciliar = async () => {
    ...
    try {
      const res = await autoConciliarImpuestos();
      setAutoResultado(res);
      await fetchData();
    } finally {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {stats.sinConciliar > 0 && (
            <button
              id="btn-auto-conciliar-impuestos"
        ...
        <div className={`p-4 rounded-2xl border flex items-start gap-3 animate-in slide-in-from-top-2 duration-300 ${
          ...
          <div className="flex-1">
            ...
            {autoResultado.errores.length > 0 && (
              <p className="text-xs text-red-600 dark:text-red-400 mt-1">{autoResultado.errores[0]}</p>
            )}
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          label={`Pagado ${new Date().getFullYear()}`}
          value={fmt(stats.pagadoAnio)}
          color="violet"
        />
        <StatCard
          icon={<Link2 className="w-5 h-5" />}
          label="Sin Conciliar"
          value={String(stats.sinConciliar)}
          color="blue"
          suffix={stats.sinConciliar > 0 ? 'impuestos' : '✓ Al día'}
        />
        <StatCard
      ...
      <div className="bg-white dark:bg-[#0a0a0a] rounded-3xl border border-gray-100 dark:border-gray-800 p-4 shadow-sm flex flex-wrap gap-3 items-center">
        ...
        <div className="relative flex-1 min-w-40">
          ...
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Buscar por concepto o período..."
            className="w-full pl-9 pr-3 py-2 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-black text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 text-sm"
          />
        </div>
          ...
          {(['Todos', 'Pendiente', 'Pagado', 'Conciliado'] as const).map(e => (
            <button
              ...
              className={`px-3 py-1.5 text-xs rounded-xl font-medium transition-all ${
                filterEstado === e
                  ? 'bg-white dark:bg-white/10 text-gray-900 dark:text-white shadow-sm ring-1 ring-gray-200 dark:ring-gray-700'
        ...
        {(search || filterTipo !== 'Todos' || filterEstado !== 'Todos') && (
          <button
            onClick={() => { setSearch(''); setFilterTipo('Todos'); setFilterEstado('Todos'); }}
            className="px-3 py-2 text-sm text-gray-500 hover:text-gray-800 dark:text-gray-400 font-medium hover:bg-gray-100 dark:hover:bg-white/5 rounded-xl transition-colors"
          >
            Limpiar Filtros
          </button>
        )}
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <div className="p-12 text-center text-gray-500 dark:text-gray-400 bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-gray-800 rounded-3xl border-dashed">
            <Landmark className="w-12 h-12 mx-auto mb-3 text-gray-300 dark:text-gray-600" />
            <p className="text-lg font-medium text-gray-900 dark:text-white mb-1">No hay impuestos</p>
            <p className="text-sm mb-6">Añade un impuesto con el botón "Añadir Impuesto".</p>
            <button onClick={() => setShowNew(true)} className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-500/20 transition-all text-sm">
              <Plus className="w-4 h-4" /> Añadir Impuesto
            </button>
          </div>
        ) : (
          filtered.map(imp => (
            <ImpuestoRow
              ...
              onDelete={() => setDeletingId(imp.id)}
              onVincular={() => setVinculando(imp)}
              onDesvincular={() => handleDesvincular(imp)}
            />
          ))
        )}
      </div>
...
/* ─── Impuesto Row ─── */
function ImpuestoRow({ imp, movimientos, onEdit, onDelete, onVincular, onDesvincular }: {
  imp: any;
  movimientos: any[];
  onEdit: () => void;
  onDelete: () => void;
  ...
  return (
    <div className={`p-5 bg-white dark:bg-[#0a0a0a] border rounded-2xl shadow-sm hover:border-indigo-100 dark:hover:border-indigo-900/50 transition-all group ${
      imp.estado === 'Pendiente'
        ? 'border-orange-100 dark:border-orange-900/30'
        : imp.estado === 'Conciliado'
        ? 'border-emerald-100 dark:border-emerald-900/30'
        : 'border-gray-100 dark:border-gray-800'
    }`}>
      <div className="flex flex-col sm:flex-row gap-4">
        {/* Icono tipo */}
        <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${TIPO_COLORS[imp.tipo as Tipo]}`}>
        ...
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <span className={`text-xs px-2 py-0.5 rounded-md font-bold ${TIPO_COLORS[imp.tipo as Tipo]}`}>
              {imp.tipo}
            </span>
            <span className={`text-xs px-2 py-0.5 rounded-md border font-semibold ${ESTADO_COLORS[imp.estado]}`}>
              {imp.estado}
            </span>
            <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">{imp.periodo}</span>
          </div>

          <h3 className="font-semibold text-gray-900 dark:text-white leading-snug">{imp.concepto}</h3>

          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
            <span className="flex items-center gap-1">
              <Calendar className="w-3 h-3" />
              Devengo: <span className="font-medium text-gray-700 dark:text-gray-300">{fmtDate(imp.fecha_devengo)}</span>
            </span>
            {imp.fecha_pago && (
              <span className="flex items-center gap-1">
                <CheckCircle2 className="w-3 h-3 text-emerald-500" />
                Pago: <span className="font-medium text-gray-700 dark:text-gray-300">{fmtDate(imp.fecha_pago)}</span>
              </span>
            )}
          </div>
          ...
          {mov && (
            <div className="flex items-center gap-2 mt-1.5 p-2 bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-100 dark:border-emerald-500/20 rounded-xl">
              ...
              <span className="text-xs text-emerald-700 dark:text-emerald-300 font-medium flex-1 truncate">
                {mov.concepto} — {fmtDate(mov.fecha)} — {fmt(mov.importe)}
              </span>
              <button
        ...
        {/* Importe + acciones */}
        <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-3 sm:pl-4 sm:border-l border-gray-100 dark:border-gray-800 shrink-0">
          ...
          <div className="flex items-center gap-2">
            {imp.estado !== 'Conciliado' && !imp.movimiento_id && (
              <button
                ...
                className="p-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-500/10 dark:hover:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20 transition-colors"
              >
            ...
            <button
              onClick={onEdit}
              title="Editar"
              className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
            >
