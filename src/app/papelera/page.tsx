...
import { FileText, CheckCircle2, Circle, Building2, Calendar, Hash, RefreshCcw, Trash2 } from "lucide-react";
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-red-600 dark:text-red-500">Papelera</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            {facturasData.length} facturas eliminadas temporalmente
          </p>
        </div>
      </div>
                ...
                <tr key={inv.id} className="hover:bg-red-50/30 dark:hover:bg-red-500/5 transition-colors group">
                  ...
                  <td className="py-3 px-4 font-mono font-medium text-indigo-600 dark:text-indigo-400">
                    <div className="flex items-center gap-1.5">
                      ...
                      {inv.numero || <span className="text-gray-400 text-xs">Sin nº</span>}
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <form action={restaurarFactura.bind(null, inv.id)}>
                        <button type="submit" title="Restaurar" className="flex items-center gap-1 px-3 py-1.5 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 dark:text-emerald-400 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20 transition-colors rounded-lg font-medium text-xs">
